...
export function PackRotator({ image, title, accent = '#d8b16a' }: Props) {
  ...
  const pointerDown = (event: React.PointerEvent<HTMLDivElement>) => { event.currentTarget.setPointerCapture(event.pointerId); drag.current = { x: event.clientX, y: event.clientY, start: { ...rotation.current } }; };
  ...
  return <div className={styles.wrap} style={{ '--pack-accent': accent } as React.CSSProperties}><div className={styles.scene} tabIndex={0} role="group" onKeyDown={keyDown} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={() => { drag.current = null; }} aria-label={isRu ? 'Интерактивная 3D-пачка NOIRA. Потяните для поворота или используйте стрелки влево и вправо.' : 'Interactive NOIRA 3D pack. Drag to rotate or use the left and right arrow keys.'}><div ref={pack} className={styles.pack} style={{ transform: `rotateX(${initial.x}deg) rotateY(${initial.y}deg)` }}>
  ...
  </div></div><div className={styles.hint}><span>{isRu ? 'Потяните, чтобы повернуть пачку' : 'Drag to rotate the pack'}</span><button type="button" onClick={reset} aria-label={isRu ? 'Сбросить поворот пачки' : 'Reset pack rotation'}><RotateCcw size={15} /></button></div></div>;
}
